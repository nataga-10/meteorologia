...
class VentanaVisualizacion(tk.Toplevel):
    ...
    def actualizar_grafica(self, predicciones, visualizador):
        """Implementación con diseño grid y elementos visuales optimizados"""
        try:
            ...
            for col, fecha in enumerate(fechas_unicas):
                for row, periodo in enumerate(periodos):
                    ...
                    if img is not None:
                        try:
                            # Convertir imagen matplotlib a PhotoImage de Tkinter
                            img_array = (img * 255).astype(np.uint8)
                            pil_image = Image.fromarray(img_array)
                            tk_image = ImageTk.PhotoImage(pil_image.resize((70, 50)))
                            
                            # Guardar referencia
                            self.tk_images.append(tk_image)
                            
                            # Mostrar imagen
                            img_label = tk.Label(inner_frame, image=tk_image, bg=confidence_styles[confidence_style]['bg'])
            ...
            # Crear canvas
            canvas = FigureCanvasTkAgg(plt.Figure(), self.main_frame)
            ...
            if hasattr(self, 'pred_frame') and self.pred_frame:
                ...
                self.pred_frame.pack(fill=tk.X, side=tk.BOTTOM)
